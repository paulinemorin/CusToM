...
for f=f_affich
    ...
    if options.mod_marker_anim || options.exp_marker_anim
        ...
        if f==f_affich(1) || (isfield(AnimateParameters,'Mode')  && isequal(AnimateParameters.Mode, 'Figure'))
            m = patch(ax,'Faces',Colors.fmk,'Vertices',Vsmk,'FaceColor','none','FaceVertexCData',Colors.C_mk,'EdgeColor','none');
            m.Marker='o';
 ...
 %           end
    ...
    if options.Stiffness && ~isempty(Muscles) && sum([Muscles.exist])
        ...
        for solid_eff=effector(:,1)'
            ...
            F_ef=[];V_ef=[];
            for i_for=1:3
                P = Human_model_bis(solid_eff).p+Human_model_bis(solid_eff).R*Human_model_bis(solid_eff).anat_position{effector(i_eff,2),2};%regarder si P a boug� ou pas, et changer la couleur, push quand l'affichage est ok
