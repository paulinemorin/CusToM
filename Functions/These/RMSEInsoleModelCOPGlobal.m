function [RMSE] = RMSEInsoleModelCOPGlobal(X,CoP_Pos,PositionThreshold,VelocityThreshold,dw,dv0,dR,Human_model,Contact_detection,Prediction,nbframe,NbPointsPrediction,Same_Xp,Same_Pred,Foot_large,Foot_length,Foot_large_insole,Foot_length_insole,GRF_Pred,GRF_Xp,p_pelvis,r_pelvis,v0,w,Contact_sum,Solids,q,dq,ddq,num_s,filename,Contact_detect)
...
for ii=1:numel(num_s)
    ...
    GRF.(Solids{ii})=zeros(nbframe,3);
end

 for i=1:nbframe
     for ii=1:numel(num_s)
         GRF.(Solids{ii})(i,:)=GRF_Xp(i).Visual(4:6,ii)-GRF_Xp(i).Visual(1:3,ii);
...
for ii=[1,2]
    ...
    CoP_Pos_G((ii-1)*3+1,:)=[taille(1).*CoP_Pos((ii-1)*2+1,:).*Contact_bool(ii,:)];
    CoP_Pos_G((ii-1)*3+2,:)=[taille(2).*CoP_Pos((ii-1)*2+2,:).*Contact_bool(ii,:)];
...
% rotation matrice quand pied droit a plat sur le sol à la frame 153
% exprimé dans repère global
R_droit = R;
loc_droit = Human_model(num_s(ii)).p;

% 232 pour le gauche
R_gauche = R;
loc_gauche = Human_model(num_s(ii)).p;


for i = 1:nbframe
    CoP_Pos_G(4:6,i)=a'*CoP_Pos_G(4:6,i);
    CoP_Pos_G(1:3,i)=a'*CoP_Pos_G(1:3,i);
end

for i = 1:nbframe
    CoP_Pos_G(4:6,i)=R_droit*CoP_Pos_G(4:6,i)+loc_droit;
    CoP_Pos_G(1:3,i)=R_gauche*CoP_Pos_G(1:3,i)+loc_gauche;
end
...
%             else
%                 % if norm(GRF_Xp(i).Visual(4:6,3-ii)-GRF_Xp(i).Visual(1:3,3-ii))>50&&Contact_sum(ii,i)>0
%                 if (GRF_Xp(i).Visual(6,3-ii)-GRF_Xp(i).Visual(3,3-ii))>50&&Contact_detection_sum(3-ii,i)>0
%                     % if  GRF_Xp(i).Visual(6,3-ii)-GRF_Xp(i).Visual(3,3-ii)>10&&Contact_detection_sum(ii,i)>1
%                     COP_Xp.(Solids{ii})(i,:)= GRF_Xp(i).Visual(1:3,3-ii);
%                     a =  [GRF_Pred(i).Visual(1:3,ii);1]';
%                     COP_Pred.(Solids{ii})(i,:) = a(1:3);
%                     a =  [inv(R_g_to_f)*insole_foot{ii}*[[[Foot_length_insole,Foot_large_insole].*CoP_Pos(1+(ii-1)*2:2+(ii-1)*2,i)']';0;1]]';
%                     %  a =  [insole_foot{ii}*[[[Foot_length_insole,Foot_large_insole].*CoP_Pos(1+(ii-1)*2:2+(ii-1)*2,i)']';0;1]]';
%                     
%                     COP_Insole.(Solids{ii})(i,:) = (a(1:3)')';
%                     if ii==1
