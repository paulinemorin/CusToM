function Contact = Contact_Insole_Plateform(filename)
...
for ii=1:numel(num_s)
    ...
    for i=1:nbframe
        if Same_Xp==1
            if (GRF_Xp(i).Visual(6,ii)-GRF_Xp(i).Visual(3,ii))>75 && Contact_detection_sum(ii,i)>0
                COP_Xp.(Solids{ii})(i,:)=GRF_Xp(i).Visual(1:3,ii);
            else
                Contact_detection_sum(ii,i)=0;
            end
        else
            if (GRF_Xp(i).Visual(6,3-ii)-GRF_Xp(i).Visual(3,3-ii))>75 && Contact_detection_sum(ii,i)>0
