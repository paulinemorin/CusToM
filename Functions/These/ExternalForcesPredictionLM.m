...
%     ub=min(X+0.45,ub_init);
    ...
    Prediction(1).RespectDynamique(i,:) = A*X -b;
    Prediction(1).RespectFrottements(i,:) = Afric*X -bfric;
    Prediction(1).RespectLimitsup(i,:) = X -ub < 0  ; %1 si cest respecte
    Prediction(1).RespectLimitslow(i,:) = X -lb > 0  ; %1 si cest respecte
...
ExternalForcesComputationResults.Prediction = Prediction;
