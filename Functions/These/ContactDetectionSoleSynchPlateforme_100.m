function [Contact_detection, CoP_Pos] = ContactDetectionSoleSynchPlateforme_100(filename, AnalysisParameters, Human_model,BiomechanicalModel, p_pelvis, r_pelvis, v0, w, dv0, dw, q, dq, ddq)
%   INPUT
...
%% interpoler 200Hz
LF_force_200 = LF_force ;
RF_force_200 = RF_force ;
...
Acc_LF = resample(Acc_LF,1,2);
Acc_RF = resample(Acc_RF,1,2);
...
if -delay_L+floor(nbframe/2)>nbframe_pressure
    delay_L=delay_R;
end

if -delay_R+floor(nbframe/2)>nbframe_pressure
    delay_R=delay_L;
end
    

RF_acc_synch=RF_acc_200(-delay_R:-delay_R+floor(nbframe/2),:);
LF_acc_synch=LF_acc_200(-delay_L:-delay_L+floor(nbframe/2),:);
...
%  start_L = - delay_L-10;
%  start_R = -delay_R-10;
%  stop_L =  - delay_L+10+floor(nbframe/2);
%  stop_R = - delay_R+10+floor(nbframe/2);
 ...
 stop_L =  - delay_L-temp+floor(nbframe/2);
 stop_R = - delay_R-temp+floor(nbframe/2);

 F_Xp.RFoot = resample(F_Xp.RFoot(:,end),1,2);
 F_Xp.LFoot = resample(F_Xp.LFoot(:,end),1,2);
...
RF_force_synch=RF_force_200(:,start_R-delay_RF:start_R-delay_RF+floor(nbframe/2)-1);
LF_force_synch=LF_force_200(:,start_L-delay_LF:start_L-delay_LF+floor(nbframe/2)-1);

delay_RF_bis = finddelay(RF_force_synch,F_Xp.RFoot(:,end));
delay_LF_bis = finddelay(LF_force_synch,F_Xp.LFoot(:,end));

if delay_RF_bis > 50
    delay_RF_bis = delay_LF_bis;
end
if delay_LF_bis > 50
    delay_LF_bis = delay_RF_bis;
end

    

RF_force_synch=RF_force_200(:,start_R-delay_RF-delay_RF_bis:start_R-delay_RF-delay_RF_bis+floor(nbframe/2)-1);
LF_force_synch=LF_force_200(:,start_L-delay_LF-delay_LF_bis:start_L-delay_LF-delay_LF_bis+floor(nbframe/2)-1);
...
plot(RF_force_200)
hold on
RF_force_200(:,1:start_R-delay_RF-delay_RF_bis)=NaN;
RF_force_200(:,start_R-delay_RF-delay_RF_bis+floor(nbframe/2)-1:end)=NaN;
plot(RF_force_200)

plot(LF_force_200)
hold on
LF_force_200(:,1:start_L-delay_LF-delay_LF_bis)=NaN;
LF_force_200(:,start_L-delay_LF-delay_LF_bis+floor(nbframe/2)-1:end)=NaN;
plot(LF_force_200)
...
Pressure_synch(1:16,:)=Pressure_200(1:16,start_R-delay_RF:start_R-delay_RF+floor(nbframe/2)-1);
Pressure_synch(17:32,:)=Pressure_200(17:32,start_L-delay_LF:start_L-delay_LF+floor(nbframe/2)-1);

CoP_synch(1:2,:)=CoP_200(1:2,start_R-delay_RF:start_R-delay_RF+floor(nbframe/2)-1);
CoP_synch(3:4,:)=CoP_200(3:4,start_L-delay_LF:start_L-delay_LF+floor(nbframe/2)-1);


figure
title('Donnée de postion du CoP')
set(gca,'color','w')
set(gcf,'color','w')
title_liste={'Pied gauche composante antero-posterieure','composante medio-laterale','Pied droit composante antero-posterieure','composante medio-laterale'};
for i=1:2
    subplot(2,2,i)
    
    plot(CoP_200(i,:),'*')
    hold on
    CoP_200(i,1:start_L-delay_LF)=NaN;
    CoP_200(i,start_L-delay_LF+floor(nbframe/2)-1:end)=NaN;
    plot(CoP_200(i,:),'*')
    title(title_liste(i))
    
    axis([0 length(CoP) -0.5 0.5])
end
...
for i=3:4
    ...
    plot(CoP_200(i,:),'*')
    hold on
    CoP_200(i,1:start_R-delay_RF)=NaN;
    CoP_200(i,start_R-delay_RF+floor(nbframe/2)-1:end)=NaN;
    plot(CoP_200(i,:),'*')
    title(title_liste(i))
