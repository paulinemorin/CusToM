function [COP_Pred] = COP_Pred_f(filename,X)
...
for ii=1:numel(num_s)
    ...
    for i=1:nbframe
        if Same_Pred==1
            if (GRF_Pred(i).Visual(6,ii)-GRF_Pred(i).Visual(3,ii))>75 && Contact_detection_sum(ii,i)>0
                COP_Pred.(Solids{ii})(i,:)=GRF_Pred(i).Visual(1:3,ii);
            else
                Contact_detection_sum(ii,i)=0;
            end
        else
            if (GRF_Pred(i).Visual(6,3-ii)-GRF_Pred(i).Visual(3,3-ii))>75 && Contact_detection_sum(ii,i)>0
