function []=PostProcessExternalForceComparison(BiomechanicalModel,AnalysisParameters,filename)
...
for ii=1:3
    ...
    xlim([1 length(F_Xp.(Solids{2})(:,ii))])
    xlabel('Frames')
    ylabel('Force (N)')
...
for ii=4:6
    ...
    xlim([1 length(M_Xp.(Solids{2})(:,ii-3))])
    xlabel('Frames')
    ylabel('Moment (Nm)')
