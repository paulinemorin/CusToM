    ...
    for k = 1:numel(num_s)
        for l = 1:size(Contact_detection,1)
            if Prediction(l).num_solid==num_s(k)
                if Same_Contact == -1
                    Contact_sum(2-k, i)=Contact_sum(k, i)+Contact_detection(l,i);
                else
...
%             COP_Pred.(Solids{ii})(i,:) =  GRF_Pred(i).Visual(1:3,ii)';
         end
    else
        if norm(GRF_Xp(i).Visual(4:6,ii)-GRF_Xp(i).Visual(1:3,ii))>50 && Contact_sum(ii,i)>0
            a =  [R_g_to_f*[GRF_Xp(i).Visual(1:3,ii);1]]';
...
%             COP_Xp.(Solids{ii})(i,:) =  GRF_Xp(i).Visual(1:3,ii)';
%             COP_Pred.(Solids{ii})(i,:) =  GRF_Pred(i).Visual(1:3,3-ii)';
        end
    end
else
    if Same_Pred==1
...
%             COP_Pred.(Solids{ii})(i,:) =  GRF_Pred(i).Visual(1:3,ii)';
        ...
        if norm(GRF_Xp(i).Visual(4:6,3-ii)-GRF_Xp(i).Visual(1:3,3-ii))>50&&Contact_sum(ii,i)>0
            a =  [R_g_to_f*[GRF_Xp(i).Visual(1:3,3-ii);1]]';
            COP_Xp.(Solids{ii})(i,:)= a(1:3);
            %COP_Xp.(Solids{ii})(i,:)= R*(GRF_Xp(i).Visual(1:3,3-ii)-Loc);
            a =  [R_g_to_f*[GRF_Pred(i).Visual(1:3,3-ii);1]]';
            COP_Pred.(Solids{ii})(i,:) = a(1:3);
        end
    end
...
%     for l = 1:size(Contact_detection,1)
%         if Prediction(l).num_solid==num_s(k)
%             Contact_sum(k, :)=Contact_sum(k, :)+Contact_detection(l,:);
%         end
%     end
% end
%
...
%% taille de pied pour normaliser
...
Foot_size=mean(vecnorm(position_HEE-position_TOE,2,2));
...
for ii=1:numel(num_s)
    ...
    %         % if  norm(GRF_Xp(jj_f).Visual(6,ii)-GRF_Xp(jj_f).Visual(3,ii))>100%norm(GRF_Xp(jj_f).Visual(4:6,ii)-GRF_Xp(jj_f).Visual(1:3,ii))>100
    %
    %         % Prediction algorithms
    %         if Same_Pred*Same_Xp==1
    %             if Contact_sum(ii,jj_f)>0
    %                 COP_Pred.(Solids{ii})(jj_f,:) = GRF_Pred(jj_f).Visual(1:3,3-ii)';
    %                 if norm(GRF_Xp(jj_f).Visual(6,ii)-GRF_Xp(jj_f).Visual(3,ii))>50
    %                     COP_Xp.(Solids{ii})(jj_f,:) = GRF_Xp(jj_f).Visual(1:3,ii)';
    %                 end
    %
    %
    %             end
    %         else
    %             if Contact_sum(3-ii,jj_f)>0
    %
    %                 COP_Pred.(Solids{ii})(jj_f,:) = GRF_Pred(jj_f).Visual(1:3,ii)';
    %                 if norm(GRF_Xp(jj_f).Visual(6,ii)-GRF_Xp(jj_f).Visual(3,ii))>10
    %                     COP_Xp.(Solids{ii})(jj_f,:) = GRF_Xp(jj_f).Visual(1:3,ii)';
    %                 end
    %
    %             end
   ...
   %     end
    ...
    RMSE_norm(ii,1)=rms((COP_Pred.(Solids{ii})(:,2)-COP_Xp.(Solids{ii})(:,2))/Foot_size);
