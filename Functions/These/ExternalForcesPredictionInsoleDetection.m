function [ExternalForcesComputationResults] = ExternalForcesPredictionInsoleDetection(filename, AnalysisParameters, BiomechanicalModel, ModelParameters)
...
suptitle(['External Forces Prediction (Threeshold): ' filename ,newline, newline])
plot(Gauche)
hold on
plot(Droite)

legend('Gauche Threshold','Droite Threshold','Gauche Insole','Droite Insole')

%X_stock=zeros(84,nbframe);
...
for i=1:nbframe
    ...
    for pred = 1:numel(Prediction)
        ...
        if Activ_Contact_Point ~= 0
            ...
            %Cpi =0.4 * Force_max_TOR(Contact_detection(pred,i),Mass);
...
ExternalForcesComputationResults.ExternalForcesPredictionInsoleDetection = external_forces_pred;
