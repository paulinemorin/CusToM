function [ExternalForcesComputationResults] = ExternalForcesPrediction_AddMarkers(filename, AnalysisParameters, BiomechanicalModel, ModelParameters)
% Prediction of ground reaction forces for unconventional floors requiring additional
% markers
...
[real_markers, ~, ~, ~, fcapture]=C3dProcessedData(filename, markers_list);
...
for i=1:nbframe
    ...
    % Position des points de contact dans le repère mobile
    for pred=1:NbPointsPrediction
        ...
        Cpi =   Force_max_TOR_AddMarkers(position_pred_2(2),position_pred_2(3),difference_vitesse,Mass,PositionThreshold,VelocityThreshold,Offset,Width);
        Fx(pred,i)=Cpi;
        Fy(pred,i)=Cpi;
        Fz(pred,i)=Cpi;
    ...
    num_proches_marker = unique(num_proches_marker); 
    Num = num2cell(num_proches_marker);
    [external_forces_pred(i).Num_Markers] = Num; % Numéro des marqueurs formant la surface de contact 
